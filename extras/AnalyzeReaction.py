...
AtomsQCR = set([tuple(i['graph'].L()) for i in list(NanoQCR.TimeSeries.values())])
...
AtomsQCP = set([tuple(i['graph'].L()) for i in list(NanoQCP.TimeSeries.values())])
...
def CommonAtoms(Nano1, Nano2):
    ...
    CGids1 = []
    CGids2 = []
    for gid1, ts1 in list(Nano1.TimeSeries.items()):
        for gid2, ts2 in list(Nano2.TimeSeries.items()):
...
if len(satoms) > 0:
    print("After optimizing endpoints, atoms", ', '.join(["%s (%s)" % (i['graph'].ef(), commadash(i['graph'].L())) for j, i in list(NanoQCR.TimeSeries.items()) if j in sgid0]), "no longer participate in the reaction")
...
def GetIntermediates(QC0, NanoQC0, QC1, NanoQC1):
    ...
    Act0 = [i['graph'].L() for i in list(NanoQC0.TimeSeries.values()) if any([j in activeall for j in i['graph'].L()])]
    Act1 = [i['graph'].L() for i in list(NanoQC1.TimeSeries.values()) if any([j in activeall for j in i['graph'].L()])]
    ...
    for actives in activegrps:
        sgid0a = [k for k, v in list(NanoQC0.TimeSeries.items()) if all([i not in actives for i in v['graph'].L()])]
        sgid1a = [k for k, v in list(NanoQC1.TimeSeries.items()) if all([i not in actives for i in v['graph'].L()])]
        # print sgid0a, sgid1a
        print("reaction    :", ', '.join(["%s (%s)" % (i['graph'].ef(), commadash(i['graph'].L())) for j, i in list(NanoQC0.TimeSeries.items()) if j not in sgid0a]), end=' ') 
        print("->", ', '.join(["%s (%s)" % (i['graph'].ef(), commadash(i['graph'].L())) for j, i in list(NanoQC1.TimeSeries.items()) if j not in sgid1a]))
        if len(sgid1a) > 0:
            print("speculators :", ', '.join(["%s (%s)" % (i['graph'].ef(), commadash(i['graph'].L())) for j, i in list(NanoQC1.TimeSeries.items()) if j in sgid1a]))
...
if DoIntermediates:
    ...
    for iframe in [int(round(i)) for i in np.linspace(0, len(M2), min(len(M2)+1, max(11, float(len(M2))/10)))]:
        ...
        if NanoEqual(NanoQCI0, NanoQCR):
            print("Skipping frame %i ..." % iframe, end=' ')
            ...
            print("Reading frame %i ..." % iframe, end=' ')
            ...
            print("Optimizing frame %i ..." % iframe, end=' ')
        ...
        NanoChk = Nanoreactor(".tmp.xyz", enhance=1.2, printlvl=-1, boring=[])
        if NanoRef != None:
            print("[opt-%i" % gix, "\x1b[91m!=\x1b[0m" if not NanoEqual(NanoRef, NanoChk) else "==", "%i]" % RefFrm, end=' ')
        else:
                ...
                if Write:
                    ...
                    if QCL.full:
                        ...
                        for last in LastFrames[::-1]:
                            ...
                            print("reoptimizing frame %i .." % last, end=' ')
...
for i in list(QCRF.Data.keys()):
    if any([j in i for j in ['qm', 'qc', 'charge', 'mult']]):
        del QCRF.Data[i]

for i in list(QCPI.Data.keys()):
    if any([j in i for j in ['qm', 'qc', 'charge', 'mult']]):
        del QCPI.Data[i]
