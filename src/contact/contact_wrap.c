...
extern PyObject *atomic_displacement_wrap(PyObject *self, PyObject *args) {
    PyArrayObject *xyzlist_, *contacts_, *results_dr_, *results_dx_;
    int traj_length, num_contacts, num_atoms, num_dims, width_contacts;
    double *results_dr;
    double *results_dx;
    const double *xyzlist;
    ...
    if (!PyArg_ParseTuple(args, "O!O!O!O!",
            &PyArray_Type, &xyzlist_, &PyArray_Type, &contacts_, &PyArray_Type, &results_dr_, &PyArray_Type, &results_dx_)) {
        return 0;
    }
    else {
        xyzlist = (const double*) xyzlist_->data;
        contacts = (const int*) contacts_->data;
        results_dr = (double*) results_dr_->data;
        results_dx = (double*) results_dx_->data;
        ...
        atomic_displacement(xyzlist, contacts, num_contacts, traj_length, num_atoms, results_dr, results_dx);
    }
    return Py_BuildValue("d", 0.0);
}
...
static PyMethodDef _contactWrapMethods[] = {
  ...
  {"atomic_displacement_wrap", atomic_displacement_wrap, METH_VARARGS},
